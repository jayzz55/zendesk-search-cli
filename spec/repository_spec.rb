...
describe Repository do
  ...
  describe '.available_records' do
    ...
    it 'returns all the available_records' do
      expect(available_records).to eq ['users', 'organizations', 'tickets']
    end
  end
    ...
    context 'when searching for users' do
      context 'with _id 1' do
        ...
        it 'returns a collection of Models::User' do
          expect(search_results).to all(be_a(Models::User))
        end
        ...
        it 'has associated_submitted_tickets' do
          expect(search_results.first.associated_submitted_tickets).to eq(
            [Models::Ticket.new(ticket_data_1)]
          )
        end

        it 'has associated_assigned_tickets' do
          expect(search_results.first.associated_assigned_tickets).to eq(
            [Models::Ticket.new(ticket_data_1)]
          )
        end

        it 'has associated_organization' do
          expect(search_results.first.associated_organization).to eq(
      ...
      context 'with _id 2' do
        ...
        it 'has no associated_submitted_tickets' do
          expect(search_results.first.associated_submitted_tickets).to be_empty
        end

        it 'has no associated_assigned_tickets' do
          expect(search_results.first.associated_assigned_tickets).to be_empty
        end

        it 'has no associated_organization' do
          expect(search_results.first.associated_organization).to be_nil
        end
      end
    end
    ...
    context 'when searching for organizations' do
      context 'with _id 101' do
        ...
        it 'returns a collection of Models::Organization' do
          expect(search_results).to all(be_a(Models::Organization))
        end
        ...
        it 'has associated_tickets' do
          expect(search_results.first.associated_tickets).to eq(
            [Models::Ticket.new(ticket_data_1)]
          )
        end

        it 'has associated_users' do
          expect(search_results.first.associated_users).to eq(
      ...
      context 'with _id 102' do
        ...
        it 'has no associated_tickets' do
          expect(search_results.first.associated_tickets).to be_empty
        end

        it 'has no associated_users' do
          expect(search_results.first.associated_users).to be_empty
        end
      end
    end
    ...
    context 'when searching for tickets' do
      context 'with _id 74341f74-9c79-49d5-9611-87ef9b6eb75f' do
        ...
        it 'returns a collection of Models::Ticket' do
          expect(search_results).to all(be_a(Models::Ticket))
        end
        ...
        it 'has associated_submitter' do
          expect(search_results.first.associated_submitter).to eq(
            Models::User.new(user_data_1)
          )
        end

        it 'has associated_assignee' do
          expect(search_results.first.associated_assignee).to eq(
            Models::User.new(user_data_1)
          )
        end
      ...
      context 'with _id 9270ed79-35eb-4a38-a46f-35725197ea8d' do
        ...
        it 'has no associated_submitter' do
          expect(search_results.first.associated_submitter).to be_nil
        end

        it 'has no associated_assignee' do
          expect(search_results.first.associated_assignee).to be_nil
        end
